...
class _NewProjectState extends State<NewProject> {
  List<PhaseField> phaseFields = [PhaseField(1)];
      ...
      try {
        ...
        for (PhaseField phaseF in phaseFields) {
          if (phaseF.phaseName!.isNotEmpty) {
            ...
            for(Task task in phaseF.tasks){
              task.taskName.isNotEmpty?tasks.add(task):null;
            }
            Phase phase = Phase(phaseF.phaseName.toString(), phaseF.phaseDescription??'', phaseF.startDate, phaseF.dueDate, false, tasks);
            phases.add(phase);
          } else {
            throw Exception();
          }
        ...
        Project project = Project(projectUid, projectNameController.text,
            dueDate.toString(), costController.text.replaceAll(' ', ''), false, phases, []);
                  ...
                  child: Wrap(
                    children: [
                            ...
                            child: Row(
                              children: [
                                IconButton(
                                    onPressed: () {
                                      setState(() {
                                        phaseFields.add(PhaseField(phaseFields.length+1));
                                      });
                                    },
                                    icon: Icon(Icons.add)),
                                VerticalDivider(
                      ...
                      for (int x = 0; x < phaseFields.length; x++)
                        Wrap(children: [
                          phaseFields[x],
                          if (x >= 0 && x <= phaseFields.length - 2) Divider(thickness: 3)
                        ])
                    ],
                  ),
                ),
